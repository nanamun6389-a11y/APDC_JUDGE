...
let TT=[];
async function loadTimetable(){if(TT.length){render();return;}const r=await fetch('timetable-data.json?v=20260720-realistic-times');const d=await r.json();TT=d.rows||[];document.getElementById('ttSummary').textContent=d.summary||'';render();}
