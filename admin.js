...
document.getElementById('ttmSave')?.addEventListener('click',async()=>{
  ...
  try{
    ...
    await Promise.all([
      set(ref(db,'timetableOverride'),payload),
      set(ref(db,'apdcPublic/timetable'),payload)
    ]);
    ttmMessage.textContent='SAVED · JUDGE / MC / LIVE / SEARCH SYNCED';
  }catch(e){
    console.error('Timetable save failed',e);
    ttmMessage.textContent=e?.message==='AUTH_REQUIRED'
      ? 'SAVE REQUIRES FIREBASE LOGIN · OPEN MC AND SIGN IN ONCE, THEN RETURN HERE'
      : 'SAVE FAILED · LOCAL BACKUP KEPT';
  }
});
