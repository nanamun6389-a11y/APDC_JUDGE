...
async function loadTimetable(){try{const [tr,pr]=await Promise.all([fetch("timetable-data.json?v=20260722-reordered-1130-v2",{cache:"no-store"}),fetch("players.json?v=20260721-mc-v6",{cache:"no-store"})]);const d=await tr.json();TT=d.rows||[];try{PLAYERS=await pr.json()}catch(_){PLAYERS=[]}renderTimetableRow();await publishLiveStatus();onValue(ref(db,"timetableOverride"),snap=>{const v=snap.val();if(v&&Array.isArray(v.rows)&&v.rows.length){const current=TT[ttIndex];TT=v.rows;ttIndex=Math.max(0,Math.min(ttIndex,TT.length-1));if(current?.no){const found=TT.findIndex(r=>String(r.no||"")===String(current.no||"")&&String(r.event||"")===String(current.event||""));if(found>=0)ttIndex=found;}renderTimetableRow();}})}catch(e){console.error(e);ttMeta.textContent="Timetable could not be loaded."}}
