...
const ttApp=getApps().length?getApps()[0]:initializeApp(firebaseConfig);
...
let TT=[];
async function loadTimetable(){if(TT.length){render();return;}const r=await fetch('timetable-data.json?v=20260722-reordered-1130-v2',{cache:'no-store'});const d=await r.json();TT=d.rows||[];document.getElementById('ttSummary').textContent=d.summary||'';try{const s=await get(ref(ttDb,'timetableOverride'));const v=s.val();if(v&&Array.isArray(v.rows)&&v.rows.length)TT=v.rows;}catch(e){console.warn(e)}render();onValue(ref(ttDb,'timetableOverride'),snap=>{const v=snap.val();if(v&&Array.isArray(v.rows)&&v.rows.length){TT=v.rows;render();}});}
