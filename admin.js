...
setTimeout(()=>{renderJudgeAllocation().catch(console.error);refreshResultEvents();const q=`?competition=${encodeURIComponent(competitionId)}`;const a=document.getElementById('certificateLiveLink');if(a)a.href=`certificate-live.html${q}`;const r=document.getElementById('liveResultsLink');if(r)r.href=`live-results.html${q}`;document.querySelectorAll('a[href="certificate-live.html"]').forEach(x=>x.href=`certificate-live.html${q}`);},500);
