...
let TT=[];
async function loadTimetable(){if(TT.length){render();return;}const r=await fetch('timetable-data.json?v=20260719-timetable');const d=await r.json();TT=d.rows||[];document.getElementById('ttSummary').textContent=d.summary||'';render();}
...
const host=document.getElementById('ttCards');host.innerHTML=rows.map(x=>`<article class="tt-card" data-start="${esc(x.start)}"><div class="tt-time">${esc(x.start)}</div><div class="tt-main"><div class="tt-topline"><span class="tt-run">RUN ${esc(x.no)}</span><span class="tt-round">${esc(x.round)}</span></div><h2>${esc(x.event).replace(/\n/g,'<br>')}</h2><div class="tt-meta">${[x.section,x.division,x.style].filter(Boolean).map(esc).join(' · ')}</div>${x.entries?`<div class="tt-info"><b>ENTRIES</b> ${esc(x.entries)}</div>`:''}${x.danceOrder?`<div class="tt-info"><b>DANCE</b> ${esc(x.danceOrder)}</div>`:''}${x.note?`<div class="tt-note">${esc(x.note)}</div>`:''}</div><div class="tt-duration">${esc(x.duration)}${x.duration?' min':''}</div></article>`).join('')||'<div class="message">No timetable results.</div>';}
document.getElementById('ttSearch').addEventListener('input',render);
document.getElementById('ttNowBtn').addEventListener('click',()=>{const cards=[...document.querySelectorAll('.tt-card')];if(!cards.length)return;const now=new Date();const cur=now.getHours()*60+now.getMinutes();let best=cards[0],bestDiff=1e9;for(const c of cards){const m=/^(\d{1,2}):(\d{2})$/.exec(c.dataset.start||'');if(!m)continue;const v=+m[1]*60 + +m[2];const diff=Math.abs(v-cur);if(diff<bestDiff){bestDiff=diff;best=c;}}best.scrollIntoView({behavior:'smooth',block:'center'});best.classList.add('tt-highlight');setTimeout(()=>best.classList.remove('tt-highlight'),1800);});